...
folder.fastSimcoal2 <- "~/embo_popgen_2025_Betty/Betty/DAY4";
...
model.admixed.pop <- function(sample_1, sample_2, sample_admix, effective_population_size_1, effective_population_size_2, effective_population_size_admixed, time_admixture, percentage_from_population_1, time_split, effective_population_size_ancestral, number_of_blocks)
{
  ...
  exe <- "fsc28"  #The software s already installed on the EMBO server so it's not an exe file
...
number_of_blocks <- 10;
percentage_from_population_1 <- 0.7;  #In the admixture event, the pop preresent 30% of the admixed pop
...
#It will scale the point of A and B populations to their positions with the individuals of C population.
  #Scale between 0 and 1 : This is what ADMIXTURE is doing.
#########################################################################
...
# What happens if the time of admixture is much older?
  # Rather than relying only on the percentage of admixture for the position of the ind C, we will also see the effect of genetic drift that happened after the admixture event.
...
Q <- as.matrix(Q(project, K = 2, run = 2))
...
read.table(file=paste(project@runs[2][[1]]@directory,project@runs[2][[1]]@G.output.file, sep=""),header=F)
